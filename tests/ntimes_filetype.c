...
int main(int argc, char **argv)
{
    ...
    int psizes[2], gsizes[2], subsizes[2], starts[2];
    ...
    if (gap == 0) {
        err = MPI_Type_contiguous(len * len, MPI_INT, &bufType); ERR
    }
    else {
        /* resize to add a gap at the end of the buffer datatype */
        MPI_Aint ub;
        MPI_Datatype contigType;

        err = MPI_Type_contiguous(len * len, MPI_INT, &contigType); ERR
        ub = (len * len + gap) * sizeof(int);
        err = MPI_Type_create_resized(contigType, 0, ub, &bufType); ERR
    ...
    buf = (int*) malloc(extent * ntimes * sizeof(int));
    k = 0;
    for (i=0; i<ntimes; i++) {
        for (j=0; j<len*len; j++, k++) buf[k] = (k + 17 + rank) % 2147483647;
        k += gap;
    }
    ...
    for (i=0; i<extent * ntimes; i++) buf[i] = 0;
